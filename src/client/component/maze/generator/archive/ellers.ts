...
export class Ellers extends MazeGenerator {
  ...
  private merge(sink: Cell, target: Cell): boolean {
    const sinkSet = this.cellSets[sink.x][sink.y];
    const targetSet = this.cellSets[target.x][target.y];

    for (const cell of this.sets[targetSet]) {
