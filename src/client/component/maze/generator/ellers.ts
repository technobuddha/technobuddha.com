...
export class Ellers extends MazeGenerator {
  ...
  private horizontalStep() {
    ...
    if (this.currentCell.x >= this.maze.width - 1) {
      if (this.currentCell.y === this.maze.height - 1) return false;
  ...
  private computeVerticals() {
    ...
    for (let set of Object.values(this.sets)) {
      ...
      if (candidates.length > 0) {
        ...
        for (let i = 0; i < numberOfCellsToConnect; i++) verticalConnections.push(candidates[i]);
      }
    }
  ...
  private merge(sink: Cell, target: Cell) {
    ...
    for (let cell of this.sets[targetSet]) this.cellSets[cell.x][cell.y] = sinkSet;
  ...
  private verticalStep() {
    ...
    if (this.cellsToConnectVertically.length === 0) {
      ...
      this.merge(this.currentCell, { x: this.currentCell.x, y: this.currentCell.y + 1 });
