...
export abstract class Maze {
  ...
  private resolveDirection(spec: CDSpecification): CellDirection {
    ...
    if ('direction' in cell) return cell;

    const adjacent = this.adjacent(cell);
    const outside = adjacent.filter((c) => !this.inMaze(c));
    if (outside.length > 0) return { ...cell, direction: randomPick(outside)!.direction };
  ...
  public draw(): void {
    if (this.context) {
      ...
      const edges: Cell[] = [];
      for (let x = 0; x < this.width; ++x) edges.push({ x, y: y0 }, { x, y: y1 });
      for (let y = 0; y < this.height; ++y) edges.push({ x: x0, y }, { x: x1, y });
      ...
      for (const edge of edges) {
        for (const outside of this.adjacent(edge).filter((cell) => !this.inMaze(cell))) {
          ...
          for (const corner of this.corners) {
            if (corner.includes(this.opposite(direction))) this.drawPillar({ ...outside, corner });
          }
        }
      }
  ...
  public sides(cell: Cell): number {
    let s = 0;
    for (const direction of this.directions) {
      if (this.walls[cell.x][cell.y][direction]) s++;
    }
    return s;
  }
  ...
  public deadEnds(): Cell[] {
    ...
    for (let x = 0; x < this.width; ++x) {
      for (let y = 0; y < this.height; ++y) {
        if (this.isDeadEnd({ x, y })) ends.push({ x, y });
      }
    }
