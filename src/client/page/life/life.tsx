...
const LifeBoard: React.FC<LifeBoardProps> = ({ boxWidth, boxHeight, start }: LifeBoardProps) => {
  ...
  const detectGlider = React.useMemo(
    () => (activeBoard: boolean[][], context: CanvasRenderingContext2D) => {
      const w = width - 1;
      const h = height - 1;
        ...
        for (const [i, row] of board.entries()) {
          for (const [j, cell] of row.entries()) {
            const neighbors = MOVES.reduce((acc, [deltaX, deltaY]) => {
              ...
              return (
                acc +
                (newX >= 0 && newX < width && newY >= 0 && newY < height && board[newX][newY] ?
                  1
                : 0)
              );
            }, 0);
  ...
  }, [board, detectGlider, height, history, move, setBoard, width]);
