...
export const Spinner: React.FC<SpinnerProps> = ({icons, speed = 10}:SpinnerProps) => {
    ...
    const segmentStyle1     = React.useMemo<React.CSSProperties>(
        () => ({transform: `translate(0, -50%) rotate(${270 - angle/2}deg)`}),
        [angle]
    );
    const segmentStyle2     = React.useMemo<React.CSSProperties>(
        () => ({transform: `translate(0, -50%) rotate(${ 90 + angle/2}deg)`}),
        [angle]
    );
    ...
    const iconBoxStyle      = React.useMemo<(i: number) => React.CSSProperties>(
        () => memoize((i: number) => ({ transform: `rotate(${-(270 + (i+0.5)*angle)}deg)` })),
        [angle]
    );
    const iconInnerStyle    = React.useMemo<(i: number) => React.CSSProperties>(
        () => memoize((i: number) => ({ transform: `rotate(${+(270 + (i+0.5)*angle)}deg)` })),
        [angle]
    );
