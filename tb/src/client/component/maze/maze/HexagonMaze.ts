...
export class HexagonMaze extends Maze {
    ...
    public rightTurn(direction: Direction): Direction[] {
        switch(direction) {
            case 'A':   return [ 'C', 'B', 'A', 'F', 'E', 'D' ];
            case 'B':   return [ 'D', 'C', 'B', 'A', 'F', 'E' ];
            case 'C':   return [ 'E', 'D', 'C', 'B', 'A', 'F' ];
            case 'D':   return [ 'F', 'E', 'D', 'C', 'B', 'A' ];
            case 'E':   return [ 'A', 'F', 'E', 'D', 'C', 'B' ];
            case 'F':   return [ 'B', 'A', 'F', 'E', 'D', 'C' ];
            default:    throw new Error(`"${direction}" is not a valid direction`);
        }
    }
    ...
    public leftTurn(direction: Direction): Direction[] {
        switch(direction) {
            case 'A':   return [ 'E', 'F', 'A', 'B', 'C', 'D' ];
            case 'B':   return [ 'F', 'A', 'B', 'C', 'D', 'E' ];
            case 'C':   return [ 'A', 'B', 'C', 'D', 'E', 'F' ];
            case 'D':   return [ 'B', 'C', 'D', 'E', 'F', 'A' ];
            case 'E':   return [ 'C', 'D', 'E', 'F', 'A', 'B' ];
            case 'F':   return [ 'D', 'E', 'F', 'A', 'B', 'C' ];
            default:    throw new Error(`"${direction}" is not a valid direction`);
        }
    }
    ...
    public move(cell: Cell, direction: Direction): CellDirection {
        if(cell.x % 2 === 0) {
            switch(direction) {
                case 'A':   return { x: cell.x,     y: cell.y - 1,  direction };
                case 'B':   return { x: cell.x + 1, y: cell.y - 1,  direction };
                case 'C':   return { x: cell.x + 1, y: cell.y,      direction };
                case 'D':   return { x: cell.x,     y: cell.y + 1,  direction };
                case 'E':   return { x: cell.x - 1, y: cell.y,      direction };
                case 'F':   return { x: cell.x - 1, y: cell.y - 1,  direction };
                default:    throw new Error(`"${direction}" is not a valid direction`);
            }
        }
        ...
        switch(direction) {
            case 'A':   return { x: cell.x,     y: cell.y - 1,  direction };
            case 'B':   return { x: cell.x + 1, y: cell.y,      direction };
            case 'C':   return { x: cell.x + 1, y: cell.y + 1,  direction };
            case 'D':   return { x: cell.x,     y: cell.y + 1,  direction };
            case 'E':   return { x: cell.x - 1, y: cell.y + 1,  direction };
            case 'F':   return { x: cell.x - 1, y: cell.y,      direction };
            default:    throw new Error(`"${direction}" is not a valid direction`);
        }
    }
    ...
    public drawWall(cd: CellDirection, color = this.wallColor): void {
        if(this.context) {
            ...
            const { x1, x2, x3, x5, x6, x8, x9, xA, y0, y1, y2, y3, y4, y5, y6, y7, y8 } = this.offsets(cd);
